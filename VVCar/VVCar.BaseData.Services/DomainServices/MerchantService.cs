...
namespace VVCar.BaseData.Services.DomainServices
{
    ...
    public class MerchantService : DomainServiceBase<IRepository<Merchant>, Merchant, Guid>, IMerchantService
    {
        ...
        private bool ActivateMerchantDataOperation(Guid merchantId)
        {
            ...
            SystemSettingRepo.Add(new SystemSetting { ID = Util.NewID(), Index = 10, Name = SysSettingTypes.WXMsg_Announcement, Caption = "公告通知模板", DefaultValue = string.Empty, SettingValue = string.Empty, IsVisible = true, IsAvailable = true, Type = ESystemSettingType.Parameter, CreatedUserID = AppContext.CurrentSession.UserID, CreatedUser = AppContext.CurrentSession.UserName, CreatedDate = DateTime.Now, MerchantID = merchantId, });
            SystemSettingRepo.Add(new SystemSetting { ID = Util.NewID(), Index = 11, Name = SysSettingTypes.WXMsg_DeliveryNotify, Caption = "订单待发货提醒模板", DefaultValue = string.Empty, SettingValue = string.Empty, IsAvailable = true, Type = ESystemSettingType.Parameter, CreatedUserID = AppContext.CurrentSession.UserID, CreatedUser = AppContext.CurrentSession.UserName, CreatedDate = DateTime.Now, MerchantID = merchantId, });
            SystemSettingRepo.Add(new SystemSetting { ID = Util.NewID(), Index = 12, Name = SysSettingTypes.WXMsg_MemberConsume, Caption = "会员消费微信通知消息模板", DefaultValue = string.Empty, SettingValue = string.Empty, IsVisible = true, IsAvailable = true, Type = ESystemSettingType.Parameter, CreatedUserID = AppContext.CurrentSession.UserID, CreatedUser = AppContext.CurrentSession.UserName, CreatedDate = DateTime.Now, MerchantID = merchantId, });
