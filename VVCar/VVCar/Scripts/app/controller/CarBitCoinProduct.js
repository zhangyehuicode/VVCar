Ext.define('WX.controller.CarBitCoinProduct', {
	...
	init: function () {
		...
		me.control({
			...
			'CarBitCoinProductEdit checkboxfield[name=IsCanCarBitCoinExchange]': {
				change: me.carBitCoinExchangeChange
			},
	...
	producttypechange: function (com, newValue, oldValue, eOpts) {
		...
		if (win.form.getForm().actionMethod == 'PUT') {
			if (newValue == 0) {
				com.up('window').down('checkboxfield[name=IsCanCarBitCoinExchange]').setDisabled(true);
				com.up('window').down('checkboxfield[name=IsCanCarBitCoinExchange]').setValue(false);
				com.up('window').down('textfield[name=DeliveryNotes]').hide();
				com.up('window').down('numberfield[name=Horsepower]').show();
			} else {
				com.up('window').down('checkboxfield[name=IsCanCarBitCoinExchange]').setDisabled(false);
	...
	carBitCoinExchangeChange: function (checkbox, newValue, oldValue, eOpts) {
		var me = this;
		if (newValue) {
			me.getCarBitCoinProductEdit().down('numberfield[name=CarBitCoin]').setDisabled(false);
			me.getCarBitCoinProductEdit().down('numberfield[name=UpperLimit]').setDisabled(false);
		} else {
			me.getCarBitCoinProductEdit().down('numberfield[name=CarBitCoin]').setDisabled(true);
			me.getCarBitCoinProductEdit().down('numberfield[name=UpperLimit]').setDisabled(true);
			me.getCarBitCoinProductEdit().down('numberfield[name=CarBitCoin]').setValue(0);
